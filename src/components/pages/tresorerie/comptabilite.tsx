...
const ComptabilitePage = () => {
  ...
  const fetchData = async (exercice, banque, type, compteDebut, compteFin, debut, fin) => {
    try {
      ...
      const bilan = await axios.get("http://localhost:8080/budget/v1/api/comptabilite/bilan", {
        params: {
          exercice, banque, type, compteDebut, compteFin, debut, fin
        }
      });
  ...
  return (
    <div className="p-6 space-y-10">
              ...
              {Object.entries(balance).map(([compte, bal]) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td
                      className={`px-4 py-2 border text-right font-semibold ${solde >= 0 ? "text-green-700" : "text-red-700"
                        }`}
                    >
            ...
            <table className="min-w-full border border-gray-200 rounded-lg overflow-hidden">
              ...
              <tbody className="text-gray-600">
                ...
                <tr className="bg-gray-100">
                  ...
                  <td
                    className={`px-4 py-2 border text-right font-bold ${compteResultat.resultat >= 0
                        ? "text-green-700"
                        : "text-red-700"
                      }`}
                  >
      ...
      <div className="p-4 bg-white shadow-md rounded-xl mt-6">
        ...
        {(() => {
          const totalActif = bilan?.totalActif || 0;
          const totalPassif = bilan?.totalPassif || 0;

          const capital = totalActif - totalPassif;

          // on clone les passifs
          const passifsAvecCapital = [...(bilan?.passifs || [])];

          // ajout automatique du capital si déséquilibre
          if (capital !== 0) {
            passifsAvecCapital.push({
              compte: {
                libelle: capital > 0 ? "Capital (équilibrage)" : "Résultat (perte)"
              },
              debit: 0,
              credit: Math.abs(capital)
            });
          }

          const totalPassifFinal = totalPassif + Math.max(capital, 0);
          const totalActifFinal = totalActif;
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <table className="min-w-full">
                  ...
                  <tbody className="text-gray-600">
                    {bilan?.actifs?.length > 0 ? (
                      bilan.actifs.map((item, index) => {
                        const montant = (item.debit || 0) - (item.credit || 0);
                  ...
                  <tfoot className="bg-gray-100 font-semibold">
                    <tr>
                      ...
                      <td className="px-4 py-2 border text-right text-green-700">
                        {totalActifFinal.toLocaleString()}
                      </td>
                    </tr>
                  </tfoot>
                </table>
              ...
              <div className="border rounded-lg overflow-hidden shadow-sm">
                ...
                <table className="min-w-full">
                  ...
                  <tbody className="text-gray-600">
                    {passifsAvecCapital.length > 0 ? (
                      passifsAvecCapital.map((item, index) => {
                        const montant = (item.credit || 0) - (item.debit || 0);
                  ...
                  <tfoot className="bg-gray-100 font-semibold">
                    <tr>
                      ...
                      <td className="px-4 py-2 border text-right text-red-700">
                        {(totalPassif + Math.max(capital, 0)).toLocaleString()}
                      </td>
                    </tr>
                  </tfoot>
                </table>
              ...
              <div className="mt-6 text-center col-span-2">
                {totalActifFinal === (totalPassif + Math.max(capital, 0)) ? (
                  <div className="inline-block px-6 py-3 rounded-lg font-semibold bg-green-100 text-green-700">
                    ✔️ Bilan équilibré
                  </div>
                ) : (
                  <div className="inline-block px-6 py-3 rounded-lg font-semibold bg-red-100 text-red-700">
                    ❌ Bilan non équilibré
                  </div>
                )}
              </div>
