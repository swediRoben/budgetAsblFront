...
const OperationForm = () => {
  ...
  const handleDebitChange = (index, value) => {
    ...
    setValue(`details.${index}.debitid`, debitId);
  };
  ...
  const handleCreditChange = (index, value) => {
    ...
    setValue(`details.${index}.creditid`, creditId);
  };
  ...
  const handleUpdate = async (operationData) => {
    ...
    const resetData = {
      ...
      details: operationData.details && operationData.details.length > 0 
        ? operationData.details.map(detail => ({
            id: detail.id,
